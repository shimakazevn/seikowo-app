...
function SearchPage() {
  ...
  useEffect(() => {
    fetch(`https://www.googleapis.com/blogger/v3/blogs/${blogConfig.blogId}/posts?key=${blogConfig.apiKey}&maxResults=500`)
      .then(res => res.json())
      .then(data => {
  ...
  const buildUrl = (pageToken = null , keyword) => {
    const baseUrl = `https://www.googleapis.com/blogger/v3/blogs/${blogConfig.blogId}/posts/search?${keyword}&key=${blogConfig.apiKey}&maxResults=10`;
// https://www.googleapis.com/blogger/v3/blogs/blogId/posts/search
    let params = [];

    if (tag) params.push(`labels=${encodeURIComponent(tag)}`);
    if (keyword.trim()) params.push(`q=${encodeURIComponent(keyword.trim())}`);

    if (year) {
      params.push(`publishedMin=${year}-01-01T00:00:00Z`);
      params.push(`publishedMax=${year}-12-31T23:59:59Z`);
    }
