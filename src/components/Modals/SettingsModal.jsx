...
const SettingsModal = ({ isOpen, onClose }) => {
  ...
  const syncWithDrive = useCallback(async () => {
    ...
    try {
      ...
      const followPosts = await getHistoryData(FOLLOW_KEY, userId);
      const mangaBookmarks = await getHistoryData(MANGA_KEY, userId);
      const data = { readPosts, followPosts, mangaBookmarks };
  ...
  const handleClearAllData = useCallback(async () => {
    ...
    try {
      ...
      await saveHistoryData('favorites', userId, []);
      await saveHistoryData('bookmarks', userId, []);
      await saveHistoryData('read', userId, []);
