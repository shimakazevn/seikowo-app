...
function MangaGlideSlider({ posts, title, sliderId = "main-glide", onLoadMore }) {
  const sliderRef = useRef(null);
  const glideInstance = useRef(null);

  // Destroy Glide trước khi số lượng post thay đổi
  useEffect(() => {
    return () => {
  ...
  useEffect(() => {
    if (posts.length > 0 && sliderRef.current) {
      ...
      glide.on('run.after', () => {
        // Nếu đang ở cuối slider, gọi onLoadMore
